...
contract MOVENS is ERC20Detailed, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20Detailed("MOVENS", "MVNS", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x4d8F4779e80d4016A4Ba0992483e0604727427af;
        treasuryReceiver = 0x89ff91471F822E4aac928054445e7234a971cB9E;
    ...
    function rebase() internal nonReentrant {
        if (inSwap) return;
        uint256 rebaseRate = 0;
        uint256 deltaTimeFromInit = block.timestamp - _initRebaseStartTime;
        uint256 deltaTime = block.timestamp - _lastRebasedTime;
        uint256 times = deltaTime.div(compoundingTime);
        uint256 epoch = deltaTime.mul(10).div(compoundingTime);
    ...
    function addLiquidity() internal nonReentrant swapping {
    ...
    function swapBack() internal nonReentrant swapping {
        ...
        if (!treasurySuccess) {
            revert("Failed to send to treasury receiver");
        }
        (bool insuranceSuccess, ) = payable(insuranceFundReceiver).call{
        ...
        if (!insuranceSuccess) {
            revert("Failed to send to insurance receiver");
        }
    }
    ...
    function setInsuranceFundFee(uint256 _insuranceFundFee) external onlyOwner {
        ...
        emit InsuranceFundFee(insuranceFundFee);
    }
